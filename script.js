...
uploadedVideosBox.append(label2,input2,button2,heading2,uploadedVideosContent)
...
subscriptionsBox.append(label3,input3,button3,heading3,subscriptionsContent)
...
async function getUserActivity(url) {
    try {
        ...
        alert("Please check entered channel ID / check getUserActivity() (for developer)");
    }
...
topicBasedSearchButton.addEventListener("click", async () => {
    try {
        ...
        let result = await fetch(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q=${searchValue}&topicId=${topicId}&type=video&key=AIzaSyCGRsNdn6uUE4ZVmmd2lPT9xGCA41x1-aE`);
...
async function getPlayListChannelResults(type, value) {
    try {
        let result = await fetch(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${value}&type=${type}&key=AIzaSyCGRsNdn6uUE4ZVmmd2lPT9xGCA41x1-aE`);
        let searchInfo = await result.json();
